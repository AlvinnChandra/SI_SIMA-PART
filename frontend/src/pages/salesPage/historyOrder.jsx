// pages/salesPage/historyOrder.jsx
...
function getLoggedInSalesName() {
    try {
        ...
        return user?.namaLengkap || user?.nama || null;
    } catch {
        return null;
    }
}
...
// ============ MODAL NOTIFIKASI (hapus butuh admin) ============
function DeleteRestrictedModal({ order, isOpen, onClose }) {
    if (!isOpen || !order) return null;
    ...
    return (
        <div className="sima-table-modal-overlay" onClick={onClose}>
            ...
            >
                ...
                <div className="sima-table-modal__body">
                    <p>
                        Pesanan <strong>{order.orderNumber}</strong> dari{" "}
                        <strong>{order.storeName}</strong> sudah berstatus{" "}
                        <strong>{order.statusLabel}</strong>.
                    </p>
                    <p className="sima-table-modal__warning-text">
                        Untuk menghapus pesanan yang statusnya sudah berubah, silakan hubungi admin.
                    </p>
                </div>
...
function Order() {
    ...
    const handleCloseRestricted = () => {
        setIsRestrictedOpen(false);
        setRestrictedOrder(null);
    };
    ...
    const filteredOrders = useMemo(() => {
        const loggedInSalesName = getLoggedInSalesName();
        ...
        const filtered = orders.filter((order) => {
            ...
            return matchKeyword && matchStatus && matchSales;
        });
